"use strict";
// ════════════════════════════════════════════════════════
// ISU World Allround Championships 2026 — Heerenveen
// ════════════════════════════════════════════════════════
...
const DISTANCES={
  v:[
    {key:"d_500",label:"500m",meters:500,divisor:1,compId:9},
    {key:"d_3000",label:"3000m",meters:3000,divisor:6,compId:11},
    {key:"d_1500",label:"1500m",meters:1500,divisor:3,compId:13},
    {key:"d_5000",label:"5000m",meters:5000,divisor:10,compId:15},
  ],
  m:[
    {key:"d_500",label:"500m",meters:500,divisor:1,compId:10},
    {key:"d_5000a",label:"5000m",meters:5000,divisor:10,compId:12},
    {key:"d_1500",label:"1500m",meters:1500,divisor:3,compId:14},
...
function trunc3(n){return Math.floor(n*1000+0.0001)/1000}
function trunc2(n){return Math.round(n*100)/100}
...
function loadInactive(){try{const d=JSON.parse(localStorage.getItem("wk_allround_inactive")??"{}");if(d.v)inactive.v=new Set(d.v);if(d.m)inactive.m=new Set(d.m)}catch(_){}}
function saveInactive(){try{localStorage.setItem("wk_allround_inactive",JSON.stringify({v:[...inactive.v],m:[...inactive.m]}))}catch(_){}}
...
function parseAPIResults(raw,g,distKey){
  ...
  return withTime.map(r=>{
    ...
    return{name,country:sk.country||"",rank:r.rank,
      ...
      time:r.time,seconds:parseTime(r.time),timeBehind:r.timeBehind||"",
...
function parseTextResults(text,g,distKey){
  ...
  for(const line of lines){
    ...
    results.push({name:cleanName(nameStr),country:cc,time:rawTime,seconds:sec,rank:results.length+1,no:0,timeBehind:behind,startLane:"",record:rec,laps:[]});
  }
...
function renderOverview(){
  ...
  el.contentArea.innerHTML=`
    <div class="overview-header">
      <h2>${state.gender==="v"?"Women":"Men"} — Allround Championship</h2>
...
function renderKlassement(){
  ...
  const cc=ds.filter(d=>standings.all.some(a=>Number.isFinite(a.seconds[d.key]))).length;
  el.contentArea.innerHTML=`
    <h2 style="font-size:1.2rem;font-weight:800;margin-bottom:2px">Allround Championship Standings — ${state.gender==="v"?"Women":"Men"}</h2>
    ...
    <div class="info-box"><strong>Sorting:</strong> most distances completed → lowest points. Points = time(s) ÷ distance factor (500m=1, 1500m=3, 3000m=6, 5000m=10, 10000m=20).</div>`;
}
